...
if uploaded_file:
    ...
    if user_query := st.chat_input("What analysis would you like to perform?"):
        ...
        8. CODE FORMATTING BUG AVOIDANCE (CRITICAL): When generating the Python `code` string, DO NOT use the literal characters `\\n` to separate lines of code, as it causes SyntaxErrors in the execution environment. Instead, you MUST use semicolons `;` to separate simple statements on a single line (e.g., `import pandas as pd; print(df.isnull().sum())`).
        """
